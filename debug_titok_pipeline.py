...
class SyntheticDataset:
    ...
    def __init__(self, num_samples: int = 1000, image_size: int = 64, return_dict: bool = True):
    ...
    def __getitem__(self, idx):
        if self.return_dict:
            return {
                ...
                'label': torch.tensor(idx % 100, dtype=torch.long)  # Dummy labels
            }
        else:
            return self.images[idx]  # Return raw tensor for TiTok training
...
class DebugTiTokPipeline:
    ...
    def train_titok_tokenizer(self, train_dataset, val_dataset):
        ...
        # Create separate datasets for TiTok training that return raw tensors
        titok_train_dataset = SyntheticDataset(num_samples=500, image_size=64, return_dict=False)
        titok_val_dataset = SyntheticDataset(num_samples=100, image_size=64, return_dict=False)

        # Create data loader for TiTok training
    ...
    def train_main_model(self, tokenizer, train_dataset, val_dataset):
        ...
        # Create minimal model config - match TiTok input size for consistency
        model_config = {
            'input_size': 64,  # Match TiTok training size (64x64)
            'patch_size': 2,
            'in_channels': 3,
            'hidden_size': 192,
            'depth': 2,
        ...
        train_config = TrainingConfig(
            ...
            learning_rate=1e-3,
            image_size=64,  # Match TiTok training size
