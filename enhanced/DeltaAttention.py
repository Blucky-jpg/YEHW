...
class DeltaNetEnhancedAttention(nn.Module):
    ...
    def __init__(
        ...
        # Use standard linear for small num_heads to avoid FP8 dimension constraints
        if num_heads < 8 or num_heads % 16 != 0:
            # FP8 requires last dimension divisible by 16, so use standard precision
            self.id_gate_proj = nn.Linear(dim, num_heads, bias=True)
            print(f"Using standard Linear for id_gate_proj due to num_heads={num_heads}")
        else:
