...
class TiTokEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Extract config values with validation (support both dict and object configs)
        if isinstance(config, dict):
            ...
            self.mlp_ratio = encoder_config["mlp_ratio"]
        else:
            encoder_config = config.model.encoder
            self.input_size = encoder_config.input_size
            self.patch_size = encoder_config.patch_size
...
class TiTokDecoder(nn.Module):
    ...
    def __init__(self, config, token_size: int):
        ...
        # Extract config values (support both dict and object configs)
        if isinstance(config, dict):
            vq_config = config["model"]["vq_model"]
            encoder_config = config["model"]["encoder"]
            self.num_latent_tokens = vq_config["num_latent_tokens"]
            self.hidden_size = encoder_config["hidden_size"]
            self.output_size = encoder_config["input_size"]
            self.patch_size = encoder_config["patch_size"]
        else:
            vq_config = config.model.vq_model
            encoder_config = config.model.encoder
...
class TiTokTokenizer(nn.Module):
    ...
    def __init__(self, config=None, **kwargs):
        ...
        # Extract configuration values based on whether config is dict or object
        if isinstance(self.config, dict):
            # Dictionary access
            self.finetune_decoder = self.config["model"]["vq_model"].get("finetune_decoder", False)
            self.quantize_mode = self.config["model"]["vq_model"].get("quantize_mode", "vq")
            self.num_latent_tokens = self.config["model"]["vq_model"]["num_latent_tokens"]
            self.token_size = self.config["model"]["vq_model"]["token_size"]
            vq_config = self.config["model"]["vq_model"]
        else:
            # Object access
            self.finetune_decoder = self.config.model.vq_model.get("finetune_decoder", False)
            self.quantize_mode = self.config.model.vq_model.get("quantize_mode", "vq")
        ...
        self.encoder = TiTokEncoder(self.config)
        self.decoder = TiTokDecoder(self.config, token_size=self.token_size)
        ...
        if self.quantize_mode == "vq":
            if isinstance(self.config, dict):
                ...
                self.quantize = VectorQuantizer(
                    ...
                    token_size=vq_config["token_size"],
                    commitment_cost=vq_config["commitment_cost"],
                    use_l2_norm=vq_config["use_l2_norm"],
                )
            else:
                # Object access
                self.quantize = VectorQuantizer(
        ...
        if self.finetune_decoder:
            ...
            self.pixel_decoder = SimplePixelDecoder(self.config)
