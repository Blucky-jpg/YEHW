...
class TiTokDecoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Project tokens to patch features
        self.token_proj = QuantizedLinear(self.hidden_size, self.hidden_size)
    ...
    def forward(self, z_quantized: torch.Tensor) -> torch.Tensor:
        ...
        # Handle upsampling from compact tokens to full patch grid
        if self.num_latent_tokens < self.num_patches:
            # Use zero padding for remaining patches
            num_masks = self.num_patches - self.num_latent_tokens
            masks = torch.zeros(B, num_masks, self.hidden_size, device=x.device, dtype=x.dtype)
            x = torch.cat([x, masks], dim=1)  # (B, num_patches, hidden_size)
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        if self.finetune_decoder:
            ...
            if self.quantize_mode == "vq":
                ...
                logvar = torch.zeros_like(mean)  # Simple case: no variance
                posteriors = self.quantize(mean, logvar)
...
class ViTEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = nn.Sequential(
            ...
            BlackwellOptimizedLinear(hidden_size, hidden_size // 2),
            nn.GELU(),
            nn.LayerNorm(hidden_size // 2),
            BlackwellOptimizedLinear(hidden_size // 2, hidden_size // 4),
        )
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_heads: int, mlp_ratio: float = 4.0):
        ...
        mlp_hidden = int(hidden_size * mlp_ratio)
        self.mlp = nn.Sequential(
            BlackwellOptimizedLinear(hidden_size, mlp_hidden),
            nn.GELU(),
            BlackwellOptimizedLinear(mlp_hidden, hidden_size),
        )
...
class TiTokDecoder(nn.Module):
    """Decoder to reconstruct images from TiTok tokens."""
    ...
    def __init__(
        ...
        self.num_patches = (output_size // patch_size) ** 2

        # Project tokens to patch features
        self.token_proj = BlackwellOptimizedLinear(code_dim, hidden_size)
        
        # Mask token for padding to full patch grid (as per TiTok paper)
        ...
        # Cross-attention for upsampling tokens to patches
        ...
        # Final convolution to image
        self.final_conv = nn.ConvTranspose2d(
            hidden_size, 3, kernel_size=patch_size, stride=patch_size
        )
    ...
    def forward(self, token_embeddings: torch.Tensor) -> torch.Tensor:
        """
        Decode token embeddings to images.

        Args:
            token_embeddings: Token embeddings (B, num_tokens, code_dim)
        ...
        # Project to hidden size
        x = self.token_proj(token_embeddings)  # (B, num_tokens, hidden_size)

        # Handle upsampling from compact tokens to full patch grid
        if self.num_tokens < self.num_patches:
            # Method 1: Use mask tokens for remaining patches (as per TiTok paper)
            num_masks = self.num_patches - self.num_tokens
            masks = self.mask_token.expand(B, num_masks, -1)  # (B, num_masks, hidden_size)
            
            # Concatenate tokens and masks
            x = torch.cat([x, masks], dim=1)  # (B, num_patches, hidden_size)
            
            # Method 2 (alternative): Use cross-attention for better upsampling
            # Create queries for all patch positions
            # queries = self.pos_embed.expand(B, -1, -1)  # (B, num_patches, hidden_size)
            # x, _ = self.cross_attn(query=queries, key=x, value=x)
        
        elif self.num_tokens > self.num_patches:
            # If we have more tokens than patches, use adaptive pooling
            x = x.transpose(1, 2)  # (B, hidden_size, num_tokens)
            x = F.adaptive_avg_pool1d(x, self.num_patches)
            x = x.transpose(1, 2)  # (B, num_patches, hidden_size)
