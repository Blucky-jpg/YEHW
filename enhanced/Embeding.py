...
from enhanced.quantization import BlackwellOptimizedLinear
...
class CodeEmbedder(nn.Module):
    """
    Discrete token embedding for TiTok 1D tokenization.

    Maps discrete token indices to dense embeddings for processing
    by the DeltaNetDiT transformer.
    """

    def __init__(self, codebook_size: int, hidden_size: int, dropout: float = 0.1):
        super().__init__()
        ...
        # MLP to process token embeddings (similar to other embedders)
        self.mlp = nn.Sequential(
            BlackwellOptimizedLinear(hidden_size, hidden_size, bias=True),
            nn.SiLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        """
        Args:
            tokens: Discrete token indices (B, num_tokens)

        Returns:
            token_emb: Token embeddings (B, num_tokens, hidden_size)
        """
        # Get embeddings for token indices
        token_emb = self.embedding(tokens)  # (B, num_tokens, hidden_size)

        # Process through MLP
        token_emb = self.mlp(token_emb)
