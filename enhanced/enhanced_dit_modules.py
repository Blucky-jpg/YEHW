...
def _delta_inner_low_rank(q_i, k_i, v_i, kb_i, S_tuple, tri_strict, eye, state_rank):
    ...
    # Low-rank state operations
    S_mult = S_b.transpose(-1, -2)
    u_i = (inv @ v_i) - ((inv @ kb_i) @ S_a) @ S_mult
    out_chunk = ((q_i @ S_a) @ S_mult) + attn_local @ u_i
    ...
    # Rank truncation via SVD on the smaller matrix
    Q_a, R_a = torch.linalg.qr(S_a_large)
    Q_b, R_b = torch.linalg.qr(S_b_large)
    M = R_a @ R_b.transpose(-1, -2)
    
    try:
        U, s_val, Vh = torch.linalg.svd(M)
    except torch.linalg.LinAlgError:
        try:
            U, s_val, Vh = torch.linalg.svd(M, driver='gesvd')  # Fallback to more robust driver
        except Exception:
            # Final fallback: add small noise to make matrix well-conditioned
            M = M + torch.eye(M.shape[-1], device=M.device, dtype=M.dtype) * 1e-6
            U, s_val, Vh = torch.linalg.svd(M)
