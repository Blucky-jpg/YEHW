...
def _delta_inner(q_i, k_i, v_i, kb_i, S, tri_strict, eye):
    ...
    # Woodbury–like iterative inverse; unrolled for speed
    for i in range(1, tri_strict.size(-1)):
        inv_slice = inv[..., i, :]
        inv[..., i, :i] = inv[..., i, :i] + (inv_slice.unsqueeze(-2) * inv[..., :i, :i]).sum(-1)
...
# Let Torch-Dynamo optimise only the numerically intensive inner routine
