...
class ContextAdaptiveGating(nn.Module):
    ...
    def _pairwise_diffs(self,
        ...
        Memory-optimized version that processes in very small chunks.
        """
        if P <= 1:
        ...
        device = path_stack.device
        dtype = path_stack.dtype
        
        # Use much smaller chunks to avoid OOM
        # Adjust based on sequence length
        if L > 8192:
            chunk_size = 512  # Very small chunks for long sequences
        elif L > 4096:
            chunk_size = 1024
        else:
            chunk_size = min(2048, L)
            
        num_pairs = P * (P - 1) // 2
        
        # Pre-allocate output tensor
        pair_diffs = torch.zeros(B, L, H, num_pairs, device=device, dtype=dtype)
        
        # Get triangular indices once
        idx = _cached_triu_idx(P, device)
        
        # Process batch and heads separately for maximum memory efficiency
        for b in range(B):
            for h in range(H):
                # Process L dimension in small chunks
                for l_start in range(0, L, chunk_size):
                    l_end = min(l_start + chunk_size, L)
                    
                    # Get small chunk: (chunk_L, D, P)
                    chunk = path_stack[b, l_start:l_end, h]  # (chunk_L, D, P)
                    
                    # Compute norms efficiently
                    # (chunk_L, P)
                    norms2 = (chunk * chunk).sum(dim=1)
                    
                    # Compute dot products efficiently using einsum
                    # (chunk_L, P, P)
                    dots = torch.einsum('ldp,ldq->lpq', chunk, chunk)
                    
                    # Compute pairwise distances
                    # Broadcasting: (chunk_L, 1, P) + (chunk_L, P, 1) - (chunk_L, P, P)
                    dist2 = norms2.unsqueeze(1) + norms2.unsqueeze(2) - 2 * dots
                    dist2 = dist2.clamp(min=0.0)
                    
                    # Extract upper triangular part
                    # (chunk_L, num_pairs)
                    chunk_pairs = dist2[:, idx[0], idx[1]]
                    
                    # Store result
                    pair_diffs[b, l_start:l_end, h] = chunk_pairs
                    
                    # Clear intermediate tensors to free memory
                    del chunk, norms2, dots, dist2, chunk_pairs
    ...
    def _path_stats(self,
        ...
        # Handle case where D dimension might be squeezed out (D=1)
        if stats.dim() == 5:  # Missing D dimension
            stats = stats.unsqueeze(-2)  # Add D dimension back: (B,L,H,4,1,P_act)
        # Since D=1 (stat_dim=1), we can squeeze it out after rearranging
        stats_flat = rearrange(stats, 'b l h s d p -> b l h (p s d)')  # (B,L,H,P_act*4*1)
        # Since d=1, this is effectively (B,L,H,P_act*4)
    ...
    def forward(
        ...
        path_outputs:   List[torch.Tensor] | None = None,
